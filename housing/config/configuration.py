...
class HousingConfig:
    ...
    def get_data_injection_config(self) -> DataIngestionConfig:
        try:
            ...
            ROOT_DIR_DATA_INJECTION=os.path.join(
                ...
                self.time_stamp)
            downloaded_url=data_injection_config_dic.get(DATA_INJECTION_DATASET_DOWNLOAD_URL_KEY)

            raw_data_dir=os.path.join(ROOT_DIR_DATA_INJECTION,
                data_injection_config_dic.get(DATA_INJECTION_RAW_DATA_DIR_KEY))

            tgz_download_dir=os.path.join(ROOT_DIR_DATA_INJECTION,
            data_injection_config_dic.get(DATA_INJECTION_TGZ_DATA_DIR_KEY))

            injected_data_dir=os.path.join(ROOT_DIR_DATA_INJECTION,
            data_injection_config_dic.get(DATA_INJECTION_INJECTED_DATA_DIR_KEY))

            train_data_dir=os.path.join(ROOT_DIR_DATA_INJECTION,
            data_injection_config_dic.get(DATA_INJECTION_INJECTED_DATA_DIR_KEY),
            data_injection_config_dic.get(DATA_INJECTION_TRAIN_DATA_DIR_KEY))

            test_data_dir=os.path.join(ROOT_DIR_DATA_INJECTION,
            data_injection_config_dic.get(DATA_INJECTION_INJECTED_DATA_DIR_KEY),
            data_injection_config_dic.get(DATA_INJECTION_TEST_DATA_DIR_KEY))

            data_injection_config=DataIngestionConfig(dataset_download_url=downloaded_url,
             tgz_download_dir=tgz_download_dir, raw_data_dir=raw_data_dir, 
             ingested_train_dir=train_data_dir, ingested_test_dir=test_data_dir)
    ...
    def get_data_validation_config(self) -> DataValidationConfig:
        try:
            data_validation_config_dic=self.config_info.get(DATA_VALIDATION_CONFIG_KEY)
            ...
            report_page_file_path=os.path.join(
                DATA_VALIDATION_ROOT_DIR,
                data_validation_config_dic.get(DATA_VALIDATION_REPORT_PAGE_FILE_NAME_KEY)
                )
    ...
    def get_data_transformation_config(self) -> DataTransformationConfig:
        try:
            data_transfomration_config_dic=self.config_info.get(DATA_TRANSFORMATION_CONFIG_KEY)
            DATA_TRANSFORMATION_ROOT_DIR=os.path.join(
                self.model_pipeline_config.artifact_dir,
                data_transfomration_config_dic.get(DATA_TRANSFORMATION_DIR_KEY),
                self.time_stamp
            )
            add_bedroom_per_room=data_transfomration_config_dic.get(DATA_TRANSFORMATION_ADD_BEDROOM_PER_ROOM_KEY)
            transformed_train_dir=os.path.join(
                DATA_TRANSFORMATION_ROOT_DIR,
                data_transfomration_config_dic.get(DATA_TRANSFORMATION_TRANSFORMED_DIR_KEY),
                data_transfomration_config_dic.get(DATA_TRANSFORMATION_TRANSFORMED_TRAIN_DIR_KEY)
            )
            transformed_test_dir=os.path.join(
                DATA_TRANSFORMATION_ROOT_DIR,
                data_transfomration_config_dic.get(DATA_TRANSFORMATION_TRANSFORMED_DIR_KEY),
                data_transfomration_config_dic.get(DATA_TRANSFORMATION_TRANSFORMED_TEST_DIR_KEY)
            )
    ...
    def get_data_model_training_config(self) -> ModelTrainerConfig:
        try:
            ...
            trained_model_file_path=os.path.join(
                ...
                model_training_config_dic.get(MODEL_TRAINING_TRAINED_MODEL_FILE_NAME_KEY),
 )
            base_accuracy=model_training_config_dic.get(MODEL_TRAINING_MODEL_BASE_ACCURACY_KEY)
            model_config_file_path=os.path.join(
                ROOT_DIR,
                model_training_config_dic.get(MODEL_TRAINING_MODEL_CONFIG_DIR_KEY),
                model_training_config_dic.get(MODEL_TRAINING_MODEL_CONFIG_FILE_NAME_KEY)
            )
            model_training_config=ModelTrainerConfig(trained_model_file_path,
             base_accuracy,
    ...
    def get_data_model_evaluation_config(self)->ModelEvaluationConfig:
        try:
            model_evaluaton_config_dic=self.config_info.get(MODEL_EVALUATION_CONFIG_KEY)
            ...
            model_evaluation_config=ModelEvaluationConfig(model_evaluation_file_path,
            self.time_stamp)
    ...
    def get_data_model_push_config(self)->ModelPusherConfig:
        try:
            ...
            model_export_dir_name=model_pushing_config_dic.get(MODEL_PUSHER_MODEL_EXPORT_DIR_KEY)
            MODEL_PUSHER_ROOT_DIR=os.path.join(
                self.model_pipeline_config.artifact_dir,
                model_pushing_config_dic.get(MODEL_PUSHER_DIR_KEY),
                self.time_stamp
            )
            model_export_dir=os.path.join(MODEL_PUSHER_ROOT_DIR,
    ...
    def get_data_model_pipeline_config(self)->TrainingPipelineConfig:
        try:
            training_pipeline_config=self.config_info.get(TRAINING_PIPELINE_CONFIG_KEY)
            artifact_dir=os.path.join(ROOT_DIR,
                training_pipeline_config.get(TRAINING_PIPELINE_CONFIG_PIPELINE_NAME_KEY),
                training_pipeline_config.get(TRAINING_PIPELINE_CONFIG_ARTIFACT_KEY)
            )
            training_pipe_line_config=TrainingPipelineConfig(artifact_dir=artifact_dir)
