...
class ReturnParams:
    ...
    def _return_params(self,model_name:str,json_file_path:str=None)->dict:
        try:
            return self.dict_params.get(model_name)
        except Exception as e:
            raise CustomException(error_msg=e, error_details=sys)
    ...
    def to_return_hyper_parameter_trained_model(self,model_name:str,x:pd.DataFrame,y:pd.DataFrame,base_accuracy=0.6):
        try:
            model_params=self._return_params(self.all_params_dict,model_name)
            model=self._return_model(self.all_models_dict,model_name)
            random=RandomizedSearchCV(model,model_params)
            print(f'moel name is ======= {model_name}')
            random.fit(x,y)
            if ramdom.best_score_ >= base_accuracy:
...
class CombineAll(ToClassifyDataUsingCluster,ReturnParams):
    ...
    def write_json(json_training_info_file_path:str,content_key:str,content_value,file_present_or_not=True):
    ...
    def to_return_best_model(self,df:pd.DataFrame,target_col_name:str,to_stote_model_path:str,test_data:pd.DataFrame,
                          json_training_info_file_path:str,cluster_file_path:str,n_clusters=4):
        ...
        for grp,data in cluster_data:
            ...
            for model_name in self.all_model_names:
                ...
                trained_model=self.to_return_hyper_parameter_trained_model(model_name=model_name,x=x_train,y=y_train)
            ...
            print(f'best model accuracy is {best_score=}')
            model_name=f"group_{grp}_{best_model_.split('(')[0]}.pkl"
            model_dict_based_grp.update( {(f'{model_name}',best_score): best_model_}  )

        for key,val in model_dict_based_grp.items():
            key=f'{to_stote_model_path}_{key}'
            self.write_json(json_training_info_file_path=json_training_info_file_path,
                            content_key=key, content_value=val)
        return model_dict_based_grp
