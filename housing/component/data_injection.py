...
class DataInjection:
    ...
    def download_data(self)->str:
        try:
            ...
            logging.info(f'downloading file from download url [{downloaded_url}] file path is  [{tgz_file_path}]')
    ...
    def extract_zip(self,tgz_file_path:str)->str:
        try:
            ...
            logging.info(f'extrct tar file start : tar file location is [{tgz_file_path}]')
            with tarfile.open(tgz_file_path,"r:gz") as file:
                file.extractall(path=raw_data_dir)
            logging.info(f'extrct tar file finish : raw data location is [{raw_data_dir}]')
            return raw_data_dir
        except Exception as e:
            raise CustomException(error_msg=e, error_details=sys)
    def split_train_and_test(self)->DataInjectionArtifacts:
        try:
            ...
            for train_index,test_index in split.split(housing_data_frame,housing_data_frame['income_cat']):
                strat_train_data=housing_data_frame.iloc[train_index,:].drop(columns="income_cat")
                strat_test_data=housing_data_frame.iloc[test_index,:].drop(columns="income_cat")
            ...
            data_injecton_artifacts=DataInjectionArtifacts(train_file_path=train_file_path
            , test_file_path=test_file_path
            , is_injected=True, message=f'Data injected successfully') 
