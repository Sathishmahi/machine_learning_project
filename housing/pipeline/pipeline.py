...
from housing.entity.artifacts_entity import DataInjectionArtifacts,FeatureEngineeringArtifacts,DataValidationArtifacts
...
class Pipeline:
    ...
    def start_data_transformations(self,data_injection_artifacts:DataInjectionArtifacts,
                                    data_validation_artifacts:DataValidationArtifacts)->FeatureEngineeringArtifacts:
        try:
            ...
            feature_engineering_artifacts=data_transformation.initiate_data_transformation()
            logging.info(f'data transformation output is [{feature_engineering_artifacts}]')
        except Exception as e:
    ...
    def run_pipeline(self):
        try:
            ...
            data_validation_artifacts=self.start_data_validation(data_injection_artifacts=data_injection_artifacts)
            if data_validation_artifacts.all_correct_or_not:
                ...
                raise Exception(f'data set not match to data schema please check data set format')
            print(f'finish daat validation {data_validation_artifacts}')
            feature_engineering_artifacts=self.start_data_transformations(data_injection_artifacts=data_injection_artifacts,
